...
void data_init() {
  ...
  for (long i = 0; i < DATA_SIZE; ++i) {
    ...
    keyType key = data[i];
    if (i > 0) {
      keyType last_key = data[i - 1];
      assert(last_key < key);
    }
    recordContent content = abs(key + i); // simulation
    recordPtr ptr = database::insert_to_database(Record(key, content));
    assert(ptr > last_ptr);
    last_ptr = ptr;
  ...
  for (long i = DATA_SIZE; i < DATA_SIZE + INSERT_SIZE; ++i) {
    ...
    keyType key = data[i];
    recordContent content = abs(key + i); // simulation
    records_to_insert.emplace_back(Record(key, content));
  }
}
